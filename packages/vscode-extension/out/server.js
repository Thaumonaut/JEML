...
// ../lsp/dist/parser/tokenizer.js
var require_tokenizer = __commonJS({
  "../lsp/dist/parser/tokenizer.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    ...
    var DIGIT = /[0-9]/;
    var LENGTH_UNIT = /^(px|rem|em|%|vh|vw|pt|ch|ex|fr)$/;
    function tokenize(source) {
      ...
      const peekStr = (n) => source.substr(offset, n);
      const emit = (kind, start, text) => {
        tokens.push({
          kind,
          text,
          range: { start, end: pos() }
      ...
      while (offset < source.length) {
        ...
        if (ch === "." && tokens.length > 0) {
          const prev = lastNonWhitespace(tokens);
          const prevEndsIdent = prev && (prev.kind === "tag-name" || prev.kind === "variant-name");
          const next1 = peek(1);
          ...
          if (prevEndsIdent && nextStartsVariant) {
            ...
            const varStart = pos();
            while (offset < source.length && /[a-zA-Z0-9_-]/.test(peek()))
              advance();
        ...
        if (ch === "&" && IDENT_START.test(peek(1))) {
          advance();
          while (offset < source.length && (IDENT_CONT.test(peek()) || peek() === "."))
            advance();
        ...
        if (ch === '"') {
          ...
          if (peek() === '"')
            ...
            error("Unterminated string", strStart);
        ...
        if (ch === "~") {
          ...
          while (peek() === " " || peek() === "	")
            advance();
          while (offset < source.length && IDENT_CONT.test(peek()))
            advance();
        ...
        if (DIGIT.test(ch) || ch === "-" && DIGIT.test(peek(1))) {
          ...
          while (offset < source.length && /[a-zA-Z%]/.test(peek()))
            advance();
        ...
        while (offset < source.length && isTextChar(peek()))
          advance();
    ...
    function prevIsIdent(tokens) {
      for (let i = tokens.length - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "whitespace" || t.kind === "newline")
          continue;
        return t.kind === "tag-name" || t.kind === "identifier" || t.kind === "variant-name" || t.kind === "attr-name";
      }
      return false;
    }
    function lastNonWhitespace(tokens) {
      for (let i = tokens.length - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "whitespace" || t.kind === "newline")
          continue;
    ...
    function isTextChar(ch) {
      return ch !== "" && ch !== "\n" && ch !== "<" && ch !== ">" && ch !== "/" && ch !== "!" && ch !== "[" && ch !== "]" && ch !== "=" && ch !== ":" && ch !== '"' && ch !== "&" && ch !== "@" && ch !== "$" && ch !== "%" && ch !== "^" && ch !== "*" && ch !== "_" && ch !== "`" && ch !== "#" && ch !== "~" && ch !== "\\" && ch !== "-";
    }
    function classifyIdentifier(text, tokens, currentMode) {
      let prev = null;
      for (let i = tokens.length - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "whitespace" || t.kind === "newline")
      ...
      if (prev && (prev.kind === "directive-open" || prev.kind === "directive-close" || prev.kind === "block-open" || prev.kind === "block-close" || prev.kind === "inline-open" || prev.kind === "inline-close" || prev.kind === "void-marker")) {
        return "tag-name";
      }
      ...
      if (currentMode === "attr-list") {
        if (prev) {
          if (prev.kind === "attr-eq")
            return "identifier";
          if (prev.kind === "attr-bracket-open" || prev.kind === "string-quoted" || prev.kind === "number" || prev.kind === "boolean" || prev.kind === "css-length" || prev.kind === "identifier" || prev.kind === "var-ref" || prev.kind === "handler-ref" || prev.kind === "responsive-close") {
            return "attr-name";
          }
        }
        return "attr-name";
...
// ../lsp/dist/parser/index.js
var require_parser = __commonJS({
  "../lsp/dist/parser/index.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.buildTokenIndex = buildTokenIndex;
    function buildTokenIndex(tokens) {
      ...
      return {
        ...
        before(line, column) {
          let best = null;
          for (const t of tokens) {
            if (t.range.end.line < line || t.range.end.line === line && t.range.end.column <= column) {
              if (t.kind === "whitespace" || t.kind === "newline")
                continue;
              best = t;
            } else {
    ...
    function contains(t, line, column) {
...
// ../lsp/dist/data/tags.js
var require_tags = __commonJS({
  "../lsp/dist/data/tags.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.GLOBAL_ATTRIBUTES = exports2.TAGS = void 0;
    exports2.lookupTag = lookupTag;
    exports2.tagsForFlavor = tagsForFlavor;
    exports2.allTagNames = allTagNames;
    exports2.TAGS = [
      // ───────── Document structure ─────────
      {
        name: "document",
      ...
      },
      {
        name: "meta",
        flavor: "block",
        description: 'Document metadata directive. Produces `<title>`, `<meta>`, and `<link rel="icon">` tags in the HTML head.',
        htmlMapping: "elements inside <head>",
        variants: [],
        attributes: ["title", "description", "icon", "scale", "author", "keywords", "viewport"],
        specSection: "\xA75.1.1"
      },
      {
        name: "style",
        flavor: "block",
        description: "Stylesheet directive. Inline CSS in the body of the directive, or external via `ref`.",
        htmlMapping: '<link rel="stylesheet"> or <style>...</style>',
        variants: [],
        attributes: ["type", "ref"],
        specSection: "\xA75.1.2"
      },
      {
        name: "import",
        flavor: "block",
      ...
      // ───────── Headings ─────────
      {
        ...
        attributes: [],
        specSection: "\xA77.5.1"
      },
      // ───────── Text ─────────
      {
        name: "text",
        flavor: "block",
        description: "A paragraph of prose. Variants modify appearance without changing the underlying `<p>` element.",
        htmlMapping: "<p>",
        variants: [
      ...
      // ───────── Lists ─────────
      {
        name: "list",
        flavor: "block",
        description: "A list of items. Use `-` sibling markers for each item. Variants control ordering and marker style.",
      ...
      },
      {
        name: "field",
        flavor: "void",
        description: "A labeled form field. The `type` attribute determines what input is generated (text, email, password, textarea, select, checkbox, radio, etc.).",
    ...
    var TAG_INDEX = new Map(exports2.TAGS.map((t) => [t.name, t]));
    function lookupTag(name) {
      return TAG_INDEX.get(name) ?? null;
    }
    function tagsForFlavor(flavor) {
      return exports2.TAGS.filter((t) => t.flavor === flavor || t.flavor === "any");
    }
    function allTagNames() {
      return exports2.TAGS.map((t) => t.name);
    }
    exports2.GLOBAL_ATTRIBUTES = ["id", "class", "style", "key", "data-*", "aria-*"];
  }
});

// ../lsp/dist/data/attributes.js
var require_attributes = __commonJS({
  "../lsp/dist/data/attributes.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.ATTRIBUTES = void 0;
    exports2.lookupAttribute = lookupAttribute;
    exports2.layoutAttributes = layoutAttributes;
    exports2.styleAttributes = styleAttributes;
    exports2.supportsResponsive = supportsResponsive;
    exports2.ATTRIBUTES = [
    ...
    var ATTR_INDEX = new Map(exports2.ATTRIBUTES.map((a) => [a.name, a]));
    function lookupAttribute(name) {
      return ATTR_INDEX.get(name) ?? null;
    }
    function layoutAttributes() {
      return exports2.ATTRIBUTES.filter((a) => a.class === "layout").map((a) => a.name);
    }
    function styleAttributes() {
      return exports2.ATTRIBUTES.filter((a) => a.class === "style").map((a) => a.name);
    }
    function supportsResponsive(attrName) {
...
// ../lsp/dist/features/diagnostics.js
var require_diagnostics = __commonJS({
  "../lsp/dist/features/diagnostics.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.computeDiagnostics = computeDiagnostics;
    var node_12 = require_node3();
    var tags_1 = require_tags();
    var attributes_1 = require_attributes();
    function computeDiagnostics(result) {
      const diagnostics = [];
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (t.kind !== "tag-name")
          continue;
        const tagDef = (0, tags_1.lookupTag)(t.text);
        if (!tagDef) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        if (tokens[i].kind !== "responsive-caret")
          continue;
        const attrName = findOwningAttribute(tokens, i);
        if (!attrName)
          continue;
        const attrDef = (0, attributes_1.lookupAttribute)(attrName.text);
    ...
    function collectVariants(tokens, tagIndex) {
      const variants = [];
      let i = tagIndex + 1;
    ...
    function findOwningAttribute(tokens, caretIndex) {
      for (let j = caretIndex - 1; j >= 0; j--) {
        const t = tokens[j];
        if (t.kind === "attr-bracket-open")
          return null;
        if (t.kind === "attr-bracket-close")
          return null;
        if (t.kind === "attr-name")
          return t;
      }
    ...
    function detectUnclosedBlocks(tokens) {
      const diagnostics = [];
      const stack = [];
      ...
      for (const entry of stack) {
        const word = entry.kind === "directive" ? "directive" : entry.kind === "block" ? "block" : "inline element";
    ...
    function classifyDirectiveOpens(tokens) {
      const result = /* @__PURE__ */ new Map();
      for (let i = 0; i < tokens.length; i++) {
        if (tokens[i].kind !== "directive-open")
          continue;
        let depth = 0;
        let sawColon = false;
...
// ../lsp/dist/features/completions.js
var require_completions = __commonJS({
  "../lsp/dist/features/completions.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.computeCompletions = computeCompletions;
    var node_12 = require_node3();
    var tags_1 = require_tags();
    var attributes_1 = require_attributes();
    function computeCompletions(ctx) {
      ...
      if (prev.kind === "block-open" || prev.kind === "inline-open" || prev.kind === "void-marker" || prev.kind === "directive-open") {
        const flavor = prev.kind === "block-open" ? "block" : prev.kind === "inline-open" ? "inline" : prev.kind === "void-marker" ? "void" : "block";
      ...
      if (inBrackets) {
        if (prev.kind === "attr-bracket-open" || prev.kind === "whitespace" || prev.kind === "string-quoted" || prev.kind === "number" || prev.kind === "boolean" || prev.kind === "identifier" || prev.kind === "css-length") {
          const tagTok = findTagForBracket(ctx.index.tokens, inBrackets);
          if (tagTok) {
            return attributeNameCompletions(tagTok.text);
          }
          return attributes_1.ATTRIBUTES.map((a) => ({
            label: a.name,
    ...
    function rootDirectiveCompletions() {
      return [
        {
          label: ">> meta",
    ...
    function tagNameCompletions(flavor) {
      return (0, tags_1.tagsForFlavor)(flavor).map((t) => ({
        label: t.name,
    ...
    function variantCompletions(tagName) {
      const tag = (0, tags_1.lookupTag)(tagName);
      if (!tag)
        return [];
    ...
    function attributeNameCompletions(tagName) {
      const tag = (0, tags_1.lookupTag)(tagName);
      const tagAttrs = tag ? tag.attributes : [];
      const tagAttrDefs = tagAttrs.map((n) => (0, attributes_1.lookupAttribute)(n)).filter((a) => a !== null);
      const globalAttrDefs = attributes_1.ATTRIBUTES.filter((a) => a.name === "id" || a.name === "key");
    ...
    function attributeValueCompletions(attrName) {
      const attr = (0, attributes_1.lookupAttribute)(attrName);
      if (!attr || !attr.enumValues)
        return [];
      return attr.enumValues.map((v) => ({
        label: v,
        kind: node_12.CompletionItemKind.Value,
        detail: `${attrName} value`
      }));
    }
    function withinAttributeList(tokens, before) {
      let depth = 0;
    ...
    function findTagForBracket(tokens, bracketOpen) {
      const idx = indexOf(tokens, bracketOpen);
      for (let i = idx - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "tag-name")
          return t;
        if (t.kind === "block-open" || t.kind === "block-close" || t.kind === "directive-open" || t.kind === "directive-close") {
          return null;
        }
      }
      return null;
    }
    function findPrecedingTag(tokens, reference) {
      const idx = indexOf(tokens, reference);
      for (let i = idx - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "tag-name" || t.kind === "variant-name") {
          if (t.kind === "variant-name")
            continue;
          return t;
        }
        if (t.kind === "whitespace" || t.kind === "variant-dot")
          continue;
        return null;
      }
      return null;
    }
    function findOwningAttributeName(tokens, eqToken) {
      const idx = indexOf(tokens, eqToken);
      for (let i = idx - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "attr-name" || t.kind === "identifier")
          return t;
        if (t.kind === "whitespace")
          continue;
        return null;
    ...
    function isVariantDotContext(ctx) {
...
// ../lsp/dist/data/sigils.js
var require_sigils = __commonJS({
  "../lsp/dist/data/sigils.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.SIGILS = void 0;
    exports2.lookupSigil = lookupSigil;
    exports2.SIGILS = {
      ">>": {
        sigil: ">>",
        name: "directive open",
        description: "**Directive open sigil.**\n\nOpens a document-level section. Directives include `meta`, `style`, `import`, `document`, and `script`. Paired with `<<` for directives that have a body, or terminated at end of line for directives that don't (like `>> meta [...]`).",
        specSection: "\xA75.1"
      },
      "<<": {
        sigil: "<<",
        name: "directive close",
        description: "**Directive close sigil.**\n\nCloses a directive opened with `>>`. Can optionally include the directive name for clarity: `<< document`.",
        specSection: "\xA75.1"
      },
      ">": {
        sigil: ">",
        name: "block open",
        description: "**Block element open sigil.**\n\nOpens a multi-line block element. Must be followed by a tag name, optional variants (`.name`), optional attributes `[...]`, then `:` to begin content.",
        specSection: "\xA75.2"
      },
      "<": {
        sigil: "<",
        name: "block close",
        description: "**Block element close sigil.**\n\nCloses a block element opened with `>`. Can optionally include the tag name for clarity: `< section`.",
        specSection: "\xA75.2"
      },
      "/>": {
        sigil: "/>",
        name: "inline open",
        description: "**Inline element open sigil.**\n\nOpens an inline element that stays on a single line within text flow. Paired with `</`. Use for elements like `strong`, `em`, `span`, `code`.",
        specSection: "\xA75.3"
      },
      "</": {
        sigil: "</",
        name: "inline close",
        description: "**Inline element close sigil.**\n\nCloses an inline element opened with `/>`.",
        specSection: "\xA75.3"
      },
      "!>": {
        sigil: "!>",
        name: "void element",
        description: '**Void element sigil.**\n\nA self-contained element with no body and no closing tag. Carries all its information in attributes. Used for `image`, `field`, `break`, `divider`, `spacer`, `icon`.\n\nExample: `!> field [type=email label="Email"]`',
        specSection: "\xA75.4"
      },
      ":": {
        sigil: ":",
        name: "content delimiter",
        description: "**Content delimiter.**\n\nMandatory character that separates tag declaration from content. Every element with content uses `:`, whether block, inline, or directive. The compiler uses this as the unambiguous signal that the tag header is complete.",
        specSection: "\xA75.5"
      },
      ".": {
        sigil: ".",
        name: "variant separator",
        description: "**Variant separator.**\n\nAttaches a variant to a tag name. Variants are enumerated sub-types: `heading.1`, `button.primary`, `card.elevated`. Each tag has a fixed list of allowed variants. Variants can stack: `button.primary.lg`.",
        specSection: "\xA77.5"
      },
      "&": {
        sigil: "&",
        name: "variable reference",
        description: "**Variable reference sigil.**\n\nReferences a value declared in the `>> script` section. Reactive \u2014 changes to the underlying variable trigger re-render.\n\nExample: `> heading.1: Hello, &username <`",
        specSection: "\xA710.1"
      },
      "@": {
        sigil: "@",
        name: "handler reference",
        description: "**Handler reference sigil.**\n\nReferences a function declared in the `>> script` section. Pass by reference to event attributes like `on_press`.\n\nExample: `> button [on_press=@increment]: Add one <`",
        specSection: "\xA710.2"
      },
      "$": {
        sigil: "$",
        name: "iterator reference",
        description: "**Iterator reference sigil.**\n\nReferences a loop-bound variable inside a `~ for` loop body. Scoped to the loop \u2014 not accessible outside.\n\nExample: `~ for (&users as $user)`",
        specSection: "\xA710.3"
      },
      "^": {
        sigil: "^",
        name: "responsive override",
        description: "**Responsive override sigil.**\n\nAttaches a breakpoint-conditional value to an attribute. Mobile-first: the base value applies below the breakpoint, the override applies at and above.\n\nExample: `cols=1 ^(768px)=2 ^(1024px)=4`",
        specSection: "\xA78.6"
      },
      "~": {
        sigil: "~",
        name: "control flow",
        description: "**Control flow sigil.**\n\nIntroduces control flow constructs: `~ for`, `~ if`, `~ else`. Paired with `~<` as the closing marker.",
        specSection: "\xA711"
      },
      "~<": {
        sigil: "~<",
        name: "control flow close",
        description: "**Control flow close sigil.**\n\nCloses a `~ for`, `~ if`, or `~ else` block.",
        specSection: "\xA711"
      },
      "%": {
        sigil: "%",
        name: "line comment",
        description: "**Line comment sigil.**\n\nEverything from `%` to end of line is a comment. Stripped at compile time.",
        specSection: "\xA713"
      },
      "%{": {
        sigil: "%{",
        name: "block comment open",
        description: "**Block comment open.**\n\nOpens a multi-line block comment. Closed with `%}`. Does not nest.",
        specSection: "\xA713"
      },
      "%}": {
        sigil: "%}",
      ...
      "-": {
        sigil: "-",
        name: "sibling marker",
        description: "**Sibling item marker.**\n\nMarks items in a list, cells in a table row, or entries in a navigation links block. Positioned at the start of a line with consistent indent.",
        specSection: "\xA76"
      },
      "#": {
        sigil: "#",
        name: "link shorthand",
        description: "**Link shorthand.**\n\nA compact form for inline links. Written as `#'text'[url=\"...\"]`.\n\nExample: `Read the #'docs'[url=\"/docs\"] for details.`",
        specSection: "\xA79.1"
      },
      "\\": {
        sigil: "\\",
        name: "escape",
        description: "**Escape character.**\n\nEscapes the next character so it is treated literally. Useful for including sigils in content: `\\*`, `\\&`, `\\_`, `\\>`.",
        specSection: "\xA713.3"
      }
    };
    function lookupSigil(text) {
      return exports2.SIGILS[text] ?? null;
    }
  }
});

// ../lsp/dist/features/hover.js
var require_hover = __commonJS({
  "../lsp/dist/features/hover.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.computeHover = computeHover;
    var node_12 = require_node3();
    ...
    function computeHover(ctx) {
      ...
      if (sigilText) {
        const doc = (0, sigils_1.lookupSigil)(sigilText);
        if (doc) {
          return {
            contents: {
...
_Spec: ${doc.specSection}_`
      ...
      if (tok.kind === "tag-name") {
        const tag = (0, tags_1.lookupTag)(tok.text);
        if (tag) {
          const variantList = tag.variants.length > 0 ? "\n\n**Variants:** " + tag.variants.map((v) => `\`.${v.name}\``).join(", ") : "";
          const attrList = tag.attributes.length > 0 ? "\n\n**Attributes:** " + tag.attributes.map((a) => `\`${a}\``).join(", ") : "";
          return {
            contents: {
              kind: node_12.MarkupKind.Markdown,
              value: `**\`${tag.name}\`** \u2014 ${tag.flavor} element

${tag.description}

**Compiles to:** \`${tag.htmlMapping}\`` + variantList + attrList + (tag.specSection ? `
...
_Spec: ${tag.specSection}_` : "")
      ...
      if (tok.kind === "variant-name") {
        ...
        if (tagTok) {
          const tag = (0, tags_1.lookupTag)(tagTok.text);
          const variant = tag?.variants.find((v) => v.name === tok.text);
          if (variant) {
            return {
              contents: {
                kind: node_12.MarkupKind.Markdown,
                value: `**\`.${variant.name}\`** variant of \`${tag.name}\`

` + variant.description + (variant.emits ? `
...
**Emits:** \`${variant.emits}\`` : "")
      ...
      if (tok.kind === "attr-name") {
        const attr = (0, attributes_1.lookupAttribute)(tok.text);
        if (attr) {
          const enumList = attr.enumValues && attr.enumValues.length > 0 ? "\n\n**Values:** " + attr.enumValues.map((v) => `\`${v}\``).join(", ") : "";
          return {
            contents: {
              kind: node_12.MarkupKind.Markdown,
              value: `**\`${attr.name}\`** \u2014 ${attr.class} attribute

` + attr.description + `

**Accepts:** ${attr.accepts.join(" | ")}` + (attr.responsive ? "\n\n\u2713 Supports responsive overrides" : "") + enumList + (attr.specSection ? `
...
Loop-bound variable, scoped to the enclosing \`~ for\` body.`
    ...
    function precedingTag(tokens, variantTok) {
      const idx = tokens.indexOf(variantTok);
      for (let i = idx - 1; i >= 0; i--) {
        const t = tokens[i];
        if (t.kind === "tag-name")
          return t;
        if (t.kind === "variant-name" || t.kind === "variant-dot")
          continue;
        return null;
      }
      return null;
...
function analyze(doc) {
  ...
  if (cached && cached.version === doc.version)
    return cached;
  const tokenResult = (0, tokenizer_1.tokenize)(doc.getText());
  const index = (0, index_1.buildTokenIndex)(tokenResult.tokens);
  const analysis = {
    version: doc.version,
    tokens: tokenResult,
    index
...
documents.onDidChangeContent((change) => {
  ...
  const diagnostics = (0, diagnostics_1.computeDiagnostics)(analysis.tokens);
...
connection.onCompletion((params) => {
  ...
  return (0, completions_1.computeCompletions)({
