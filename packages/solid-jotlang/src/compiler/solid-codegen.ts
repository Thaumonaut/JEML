/**
 * AST → TSX codegen for solid-jotlang. Lowers a parsed `.jot` document into a
...
export function compileAst(ast: JotlDocument, options: CompileOptions = {}): CompileResult {
  ...
  if (metaDirectives.length > 0) {
    if (ctx.metaMode === 'solid-meta') {
      ...
      console.warn(
        `[solid-jotlang] ${filename}: dropping >> meta directive — install \`solid-meta\` and pass meta: 'solid-meta' to opt in.`,
      )
    }
  }
  ...
  lines.push('// AUTO-GENERATED by solid-jotlang — do not edit.')
  const solidImport = renderSolidImport(ctx.imports)
  if (solidImport) lines.push(solidImport)
  if (needsEnsureStyleImport) {
    lines.push("import { ensureGlobalStyle as __jotlEnsureStyle } from 'solid-jotlang/runtime'")
  }
...
export type { JotlDocument, DirectiveNode } from 'jotlang'
